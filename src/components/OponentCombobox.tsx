...
type Props = {
  pokemons: Pokemon[];
  setSelectedOponentId: React.Dispatch<React.SetStateAction<number | null>>;
};

export default function OponentCombobox({
  pokemons,
  setSelectedOponentId,
}: Props) {
  ...
  return (
    <Combobox.Root
      items={pokemons}
      itemToStringLabel={(value: Pokemon) => value.name}
      onValueChange={(value: Pokemon | null) =>
        setSelectedOponentId(value?.id || null)
      }
    >
      <div className="relative flex flex-col gap-1 text-sm leading-5 font-medium text-gray-900 w-full max-w-[350px] mx-auto">
        <div className="relative [&>input]:pr-[calc(0.5rem+1.5rem)] has-[.combobox-clear]:[&>input]:pr-[calc(0.5rem+1.5rem*2)]">
          <Combobox.Input
            ...
            // id={id}
            className="h-10 w-full rounded-md font-normal border border-gray-200 pl-3.5 text-base text-gray-900 bg-[canvas] focus:outline focus:outline-2 focus:-outline-offset-1 focus:outline-blue-800"
          />
